...
- 3Dモデル・アニメーション演出
- アクセス解析（自前集計・管理者ページ `/maroinudayo/`）
- ダークモード切替
...
Cloudflare D1 にページ表示と滞在時間を自前で記録し、管理者用ページ `/maroinudayo/` から確認できる。
記事ごとの閲覧数は記事一覧と記事ページにも表示される。

滞在時間はタブが手前にある間だけを加算するため、開きっぱなしのタブで平均が歪まない。

外部の解析サービスを使わず、個人を特定する情報（IP / UA / Cookie）を保存しない設計にしている。
記録するのは「パス × 日付 × 表示回数」のみ。

セットアップ手順は [docs/analytics-setup.md](docs/analytics-setup.md) を参照。
